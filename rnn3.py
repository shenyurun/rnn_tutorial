...
class RNN(object):
    ...
    def __init__(self, input, n_in, n_hidden, n_out, activation=T.tanh,
        ...
        W_init = np.asarray(np.random.uniform(size=(n_hidden, n_hidden),
                                              low=-np.sqrt(1./n_hidden), high=np.sqrt(1./n_hidden)),
                                              dtype=theano.config.floatX)
        self.W = theano.shared(value=W_init, name='W')
        # input to hidden layer weights
        W_in_init = np.asarray(np.random.uniform(size=(n_in, n_hidden),
                                                 low=-np.sqrt(1./n_in), high=np.sqrt(1./n_in)),
                                                 dtype=theano.config.floatX)
        ...
        #h_shape = self.h.get_value(borrow=True).shape
...
class MetaRNN(object):
    def __init__(self, n_in=5, n_hidden=50, n_out=5, learning_rate=0.01,
                 ...
                 final_momentum=1.0, initial_momentum=0.9,
    ...
    def shared_dataset(self, data_xy):
        ...
        shared_x = theano.shared(np.array(data_x, dtype=object))
        shared_y = theano.shared(np.array(data_y, dtype=object))
    ...
    def gradient_check_theano(self, h, X_train, Y_train):
        """ check the bptt's correctness """
        learning_rate = 0.05
        momentum = 0.9
        ...
        compute_gparams = theano.function(inputs=[index, ],
                                              outputs=gparams,
                                              givens={
                                                  self.x: train_set_x[index],
                                                  self.y: train_set_y[index]},
        ...
        myCost = train_model(0, learning_rate, momentum)
        bptt_gparams = compute_gparams(0)
        print "gradient:"
        for gparam in bptt_gparams:
	          print gparam
        print "end gradient"
        ...
        for weight in weights:
	          print weight
        print "end params"
        for (pidx, weight) in enumerate(self.rnn.params):
            parameter = weight.get_value()
            it = np.nditer(parameter, flags=['multi_index'], op_flags=['readwrite'])
            while not it.finished:
                ix = it.multi_index
                original_value = parameter[ix]
                parameter[ix] = original_value + h
                weight.set_value(parameter)
                grad_plus = compute_train_error(0)
                parameter[ix] = original_value - h
                weight.set_value(parameter)
                grad_minus = compute_train_error(0)
                estimated_grad = (grad_plus - grad_minus) / (2*h)
                parameter[ix] = original_value
                weight.set_value(parameter)
                bptt_grad = bptt_gparams[pidx][ix]
                relative_error = np.abs(bptt_grad - estimated_grad) / (np.abs(bptt_grad)+np.abs(estimated_grad))
                print "estimated gradient: %s, bptt gradient: %s, relative error: %s" % (estimated_grad, bptt_grad, relative_error)
                it.iternext()
    ...
    def fit(self, X_train, Y_train, X_test=None, Y_test=None,
        ...
        compute_train_error = theano.function(inputs=[index, ],
                                              ...
                                              givens={
                                                  ...
                                                  self.y: train_set_y[index]},
                                              mode=mode)
        compute_y_out = theano.function(inputs=[index, ],
        ...
        while (epoch < self.n_epochs):
            ...
            for idx in xrange(n_train):
                ...
                example_cost = train_model(idx, self.learning_rate,
                                           effective_momentum)
                #print self.rnn.hidden_value().eval()
                ...
                if iter % validation_frequency == 0:
                    ...
                    train_y_out = [compute_y_out(i) for i in xrange(n_train)]
                    print "y_out:"
                    for y_out in train_y_out:
                        print y_out 
                    print "end y_out"
...
def test_real():
    ...
    model = MetaRNN(n_in=n_in, n_hidden=n_hidden, n_out=n_out,
                    learning_rate=0.05, learning_rate_decay=0.999,
...
#           input_x = np.random.randint(n_classes)
...
#   print targets
    ...
    n_hidden = 40
    n_in = 4000
    n_out = 4000  # restricted to single softmax per time step
    ...
    model.fit(X_train, y_train, X_val, y_val, validation_frequency=1)
    #model.gradient_check_theano(0.1, seq, targets)
#
#    seqs = xrange(10)
#
#    plt.close('all')
